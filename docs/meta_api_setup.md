...
**→ `marketing/instagram_account_setup.md`로 이동함.** 계정 세팅, 해시태그, 캡션 SEO, 대체 텍스트, 자동 자막 등 마케팅/노출 관련 실행 가이드는 그 문서를 참고. 이 문서(`docs/meta_api_setup.md`)는 API 연동 개발 관련 내용만 다룬다.
...
마케팅/노출 최적화 관련 출처는 `marketing/instagram_account_setup.md` 참고.
